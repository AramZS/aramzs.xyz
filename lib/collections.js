...
module.exports = function loadCollection(eleventyConfig) {
	...
	const contentPaginatedByYearMonth = (collection) =>
		Array.from(
            ...
            if (post?.data?.tags && post?.data?.tags.length > 0 && post?.data?.tags.includes('writing')) {
              usePost = true;
            }
		...
		Array.from(
            ...
            var usePost = false;
						usePost = ["bookmarks", "amplify"].includes(
							post.data.permalinkBase
						) === true
            if (post?.data?.tags && post?.data?.tags.length > 0 && post?.data?.tags.includes('amplify')) {
              usePost = true;
            }
            return usePost;
          }
