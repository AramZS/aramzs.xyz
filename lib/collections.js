...
// @todo add test
const paginateContentTaxonomy = (baseSlug = "", perPage = 40) => {
...
module.exports = function loadCollection(eleventyConfig) {
    ...
    const microblogsPaginatedByYearMonth = (collection) =>
      Array.from(
        post(collection)
          .filter(
            (post) => {
              var usePost = false;
              usePost = ["microblog"].includes(
                post.data.contentType
              ) === true
              return usePost;
            }
          ...
          .reduce((carry, post) => {
            ...
            const month = post.date.getMonth() + 1;
            const segment = carry.get(key) ?? {
              title: `Microblogged in ${post.date.toLocaleString("en-us", {
                month: "long",
              })}/${post.date.getFullYear()}`,
              slug: `${post.date.getFullYear()}/${padStart(
                month,
  ...
  const musicPreview = (collection) =>{
    ...
    microblogsPaginatedByYearMonth,
		nowUpdates,
		resources,
