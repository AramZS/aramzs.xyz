...
module.exports = function loadCollection(eleventyConfig) {
	...
	const contentTags = (collection) =>
		Array.from(
			post(collection).reduce((tags, post) => {
				if (post.data.tags)
					post.data.tags.filter((e) => e).forEach(
						(tag) => tag && !tag.includes("list/") && tags.add(tag.trim().toLowerCase())
					);
				return tags;
			}, new Set())
		)
    ...
    const amplifiesPaginatedByYearMonth = (collection) =>
		Array.from(
            ...
            if (post?.data?.tags.includes('amplify')) {
              usePost = true;
            }
            return usePost;
          }
	...
	const collectSpecialTaggedContent = (prefix, collection) =>
		Array.from(
			post(collection).reduce((lists, post) => {
				if (post.data.tags) {
					for (const tag of post.data.tags.filter((e) => e )) {
						if (!tag || !tag.includes(prefix)) continue;
						lists.add(tag);
					}
				}
				return lists;
  ...
  const musicPreview = (collection) =>{
    ...
    specificMusic.items = specificMusic.items.slice(0, 8);
