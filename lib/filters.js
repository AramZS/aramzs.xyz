...
const whereKeyEquals = (collection, key, value) => {
	return betterFilter((item) => item[key] === value || (item.data && item.data[key] === value), collection);
  return collection.filter(
		(item) => item[key] === value || (item.data && item.data[key] === value)
	);
}
const whereKeyFalse = (collection, key) =>{
	return betterFilter((item) => item[key] === false || typeof item[key] === "undefined", collection)
 ...
 * @return {boolean}
 */
const isSpecialTag = (tag) => tag ? tag.includes("/") : false;
...
const excludeStubs = (collection) =>{
  ...
  return betterFilter((item) => item.data?.growthStage && item.data.growthStage !== "stub", collection)
...
const excludeType = (collection, type) => {
  if (!type) {
    ...
    return betterFilter((item) => {
      return item.data.contentType !== type;
    }, collection);
...
const excludeTypes = (collection, types = [], includes = []) =>{
	if (types.length == 0) {
    return collection;
  }
  return betterFilter((item) => {
    let result = true;
    result = types.includes(item.data.contentType) === false;
    if (item.data.hasOwnProperty('mediaType')) {
      result = false;
    }
    item.data.contentType != 'amplify' && includes && includes.forEach((tagName) => {
      if (item.data.tags.includes(tagName)){
        result = true;
      }
    });
    return result;
...
const onlyGrowthStages = (collection, stages) => {
	...
	return betterFilter((item) =>
		stages.includes(item.data.growthStage ?? "unknown"), collection)
  return collection.filter((item) =>
...
const onlyTypes = (collection, types = []) =>
	types.length > 0
		? betterFilter((item) => types.includes(item.data.contentType), collection) : collection;
...
const onlyType = (collection, type) => {
	return !type
		? collection
		: betterFilter((item) => item.data.contentType === type, collection)
    
    //collection.filter((item) => item.data.contentType === type);
};

// Neither of these 'featured' filters are used in the current site.
// They will need to be tested if I use them, 
// as I switched to `betterFilter` here without any testing.
...
const includesTag = (tags, slug) =>
	tags ? tags.find((tag) => tag.toLowerCase() === slug.toLowerCase()) !== undefined : false;
...
const excludeSpecialTags = (tags) =>
	tags ? tags.filter((tag) => tag.includes(":") === false) : [];

/**
 * Filters collection by tag
 * @param {*} collection
 * @param {Array<string>|string} onlyTags
 */
const onlyTagged = (collection, onlyTags) => {
...
const groupByKey = (collection, key) => {
  return collection.reduce((carry, post) => {
    if (post.data?.contentType == 'amplify') {
      //return carry;
    }
    if (post.data.hasOwnProperty('mediaType') && key != 'mediaType') {
      //return carry;
    }
 ...
 */
const inWeek = (collection, week, year) => {
...
const mixedDataSortWatchedMedia = (arrayOfObjects) => {
	let standardizedArrayOfObjects = arrayOfObjects.map((item) => {
		if (item.data) {
      let date = '';
      if (item.data?.watchedDate) {
        date = item.data.watchedDate;
      } else {
        date = item.date;
      }
			let newObject = {
	...
	const sorted = dateSort(standardizedArrayOfObjects, "date");
	return sorted;
};
 ...
 * @return {*}
 */
const trackBacklink = (linked, linkedFrom, title = undefined) => {
...
const countByKey = (collection, key) => {
  return collection.reduce((carry, post) => {
    if (post.data.contentType == 'amplify') {
      return carry;
    }
    ...
    carry[value].count = carry[value].count + 1;
    return carry;
  }, {});
};

// If you don't use the `safe` filter everything but the `&` should already be properly escaped. 
const escapeForRSS = (str) => {
