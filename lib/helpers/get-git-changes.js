...
async function getChanges(data) {
  ...
  if (cache.has('git-status')) {
    ...
    //console.log(chalk.blue('[@photogabble/git-history]'), 'Load cache');
  } else {
    status = await git.status();
    cache.set('git-status', status, 1200);
    console.log(chalk.blue('[@photogabble/git-history]'), 'Reset cache status');
  }
  ...
  if (/resources\/(books|film|quotes|tv|amplify|music|bookmarks)/.test(filePathname)) return [];

  if (/\/amplify\//.test(filePathname)) return [];
  ...
  try {
    ...
    console.log(chalk.blue('[@photogabble/git-history]'), 'Cache skipped for: [' + chalk.green(filePathname) + ']');
    return null;
  }
}
