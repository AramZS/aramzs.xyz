...
module.exports = {
  ...
  parseCollectionHashtags: (md, tagAtlas) => (item) => {
    if (!item.data.hashtagsMapped) {
      ...
      const found = new Set();
      const contentFull = item.template?.read(); 
      const content = contentFull?.frontMatter?.content;
      ...
      if (item.data.tags) item.data.tags.forEach((tag) => {
        if (!tag){
          console.error('tag issues', tag, item.data.tags, item.title);
        }
        return tagAtlas.findOrCreateByTitle(tag)
      });
    }
