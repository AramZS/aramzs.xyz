...
module.exports = async function () {
	...
	const numberOfPages = await fetchUrl(
		`https://${buildData.bookwyrm.instance}/user/${username}/outbox`
	)
		.then((res) => res.json())
		.then((json) => {
	...
	const fetchPage = async (page) => {
		return await fetchUrl(
			`https://${buildData.bookwyrm.instance}/user/${username}/outbox?page=${page}`
		)
			.then((res) => res.json())
			.then((json) => {
				if (
